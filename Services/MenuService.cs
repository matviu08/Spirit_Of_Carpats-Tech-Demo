    ...
    public class MenuService : IMenu
    {
        ...
        public void Update(ref GameState state, GameSettings settings, Music ambientMusic)
        {
            ...
            else if (state == GameState.Settings)
            {
                ...
                else
                {
                    ...
                    if (IsButtonClicked(settings.IsEnglish ? "Back" : "Назад", 270, 350))
                        state = GameState.MainMenu;
                }
            }
            else if (state == GameState.Chapters)
            {
                if (IsButtonClicked(settings.IsEnglish ? "Back" : "Назад", 325, 530))
                    state = GameState.MainMenu;
            }
        }
        ...
        public void Draw(GameState state, GameSettings settings)
        {
            ...
            else if (state == GameState.Settings)
            {
                ...
                DrawMenuButton(settings.IsEnglish ? "Language: English" : "Мова: Українська", 70, 660);
        ...
        private void UpdateElementsBounds()
        {
            ...
            _musicRect = new Rectangle(
                centerX - iconSize / 2,
                audioY - iconSize + (125 * scale),
                iconSize, iconSize);

            float displayY = audioY + (120 * scale);
            _lampRect = new Rectangle(
                centerX - iconSize / 2,
                displayY - iconSize + (160 * scale),
                iconSize, iconSize);
        }

        // FIX: Draw і IsButtonClicked тепер використовують однаковий fontSize і textSize
        // — раніше Draw рахував fontSize*16, а IsButtonClicked fontSize*30 → клікзона не збігалась
        private void DrawMenuButton(string text, float yOffset, float yOffset2)
        {
            ...
            DrawTextEx(_myFont, text, new Vector2(xPos, yPos), fontSize, 2,
                isHovered ? Color.Gold : Color.Beige);
        }

        private bool IsButtonClicked(string text, float yOffset, float yOffset2)
        {
            float scale    = GetScreenHeight() / 768f;
            int   fontSize = (int)(22 * scale);  // FIX: той самий розмір що й у DrawMenuButton
            Vector2 textSize = MeasureTextEx(_myFont, text, fontSize, 2);
            ...
            switch (_displayMode)
            {
                ...
                case 1:
                    ...
                    SetWindowSize(
                        ...
                        GetMonitorHeight(GetCurrentMonitor()) - 40);
                    break;
